...
const VisitUser = () => {
  ...
  useEffect(() => {
    axios.get(`http://13.126.195.239:8080/api/users/${userId}`, {
      headers: {
  ...
  const addFriend = async () => {
    try {
      const response = await axios.post('http://13.126.195.239:8080/api/users/add-friend', { friendEmail: user[0].email }, {
        headers: {
          'Authorization': localStorage.getItem('token')
        }
      });
...
<div className="p-5 border rounded text-center text-gray-500 mb-6">
        ...
        <div className="flex mt-4 justify-center">
          <button onClick={addFriend} className="bg-green-500 text-white py-2 px-4 rounded ml-4">Add Friend</button>
        </div>
      </div>
